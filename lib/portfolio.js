...
/** Reorder items within one type section and return a flat list with updated positions. */
export function mergePortfolioSectionOrder(portfolio, type, sectionItems) {
  const sections = getPortfolioSections(portfolio).map((section) =>
    section.type === type ? { ...section, items: sectionItems } : section
  );
  return sections
