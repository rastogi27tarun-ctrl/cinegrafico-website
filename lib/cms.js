...
export async function getContent() {
  noStore();
  const [hero, about, contact, hiring, services, portfolio, clients, testimonials, teamMembers] = await Promise.all([
    ...
    db.testimonial.findMany({ orderBy: { position: "asc" } }),
    db.teamMember.findMany({ orderBy: { position: "asc" } })
  ]);

  return { hero, about, contact, hiring, services, portfolio, clients, testimonials, teamMembers };
}
